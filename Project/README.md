...
Cette étude de cas met en œuvre une architecture microservices complète et résiliente basée sur Spring Cloud.
Elle intègre la découverte des services avec Eureka, un point d’entrée unique via API Gateway, la communication inter-services avec Feign Client, et la tolérance aux pannes grâce à Hystrix.

Les microservices commandes et produits, chacun relié à sa propre base H2, sont configurés via un serveur de configuration centralisé et supervisés à l’aide de Spring Boot Actuator et Swagger/OpenAPI.
Les tests Postman ainsi que l’interface utilisateur confirment la cohérence, la robustesse et le bon fonctionnement global de l’architecture.
